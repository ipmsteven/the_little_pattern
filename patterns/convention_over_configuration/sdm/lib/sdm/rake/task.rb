...
module Sdm
  module Rake
    ...
    class Task < ::Rake::TaskLib
      ...
      # DEFAULT_PATTERN = './config/size.yml'
...
#         @pattern       = DEFAULT_PATTERN
    ...
    private
      ...
      def define(args, &task_block)
        size_config = YAML.load_file('config/size.yml')
        size_config.each do |size, range|
          namespace name do
            desc "#{size} #{range}" #unless ::Rake.application.last_comment

            task size, *args do |_, task_args|
              puts "run #{size} #{range}"
